...
client.on("guildCreate", guild => {
  ...
  let lapetxs = guild.owner
  
const lptxs = new Discord.MessageEmbed()
.setTitle(`Merhaba!`)
.setColor("GREEN")//lapetxs
.addField('Prefixim: s!')
.addField(`Destek Sunucusu`, `https://discord.gg/ZDfUWKbr92 `)
lapetxs.send(lptxs)
});
...
client.on('ready', () => {
 ...
 setInterval(function() {
      ...
      let kanal = client.channels.cache.get('858486253326303243')

      let embed = new Discord.MessageEmbed()

      .setTitle(':white_check_mark: | Bot Aktifliği Sürüyor ...')
...
client.on("message",message=>{
    if(message.content==`<@!${client.user.id}>`) return message.channel.send(`Prefixim : **${prefix}** \n Beni Davet Etmek İçin **s!davet** Yazabilirsin. \n Yardım İçin **s!yardım** Yazabilirsin.`);
})

client.on('guildCreate', guild => {

    let asreaper1 = guild.channels.filter(c => c.type === "text").random()
    asreaper1.send("**Spike Botunu Sunucuya Eklediğiniz İçin Teşekkürler!** \n s!yardım Yazarak Yardım Menüsüne Erişebilirsiniz!");

});
client.on("guildCreate", async guild => {
  const asreaper2 = [
    "Botumuzu Sunucuna Eklediğin İçin Teşekkürler!",
    "Bu Bot BlackBuLL Botunun Kurucusu İle Aynı Kişidir.",
    's!yardım Yazarak Yardım Menüsüne Ulaşabilirsin'
  ];
...
client.on("message", async msg => {
  if (msg.content.toLowerCase() === 'H') {
    msg.reply('**Bot Yeniden Başlatma İptal Edildi**');
  }
});
...
const kanal = "863742765900103690" 
...
const owner = client.users.cache.get(guild.ownerID)
const kanal = "858486270261854249" 
const candycode = new Discord.MessageEmbed()
.setTitle(`📤Bir Sunucudan Çıkdım/Atıldım`)
.setColor("RED")
.addField(`Sunucu Adı`, guild.name)
.addField(`Sunucu Sahibi`, owner.username + "#" + owner.discriminator)
.addField(`Sunucu Üye Sayısı`, guild.memberCount)
...
let pinkcode = "863467085563822131"

if (guild.memberCount <  3) { //kişi sınırını ayarlayabilirsiniz

guild.leave()

return client.channels.cache.get(pinkcode).send("Eklendiğim Sunuculardan 1 Tanesi 3 Kişi ve/veya Az Kişi Oldukları İçin Fake Olduğunu Düşündüm. Bu Yüzden Sunucudan Derhal Çıktım.")
};
});
